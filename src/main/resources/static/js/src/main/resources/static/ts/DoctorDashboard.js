...
const DoctorDashboard = () => {
    ...
    const createCenterSquareCroppedFile = async (file) => {
    ...
    const handleProfileImageUpload = async (file) => {
        ...
        try {
            ...
            if (res.ok) {
                const data = await res.json();
                setProfileForm((prev) => ({ ...prev, image: data.image }));
                setDoctor((prev) => ({ ...prev, image: data.image }));
    ...
    const handleSelectPendingImage = (file) => {
        if (!file)
            return;
        if (pendingImagePreview)
            URL.revokeObjectURL(pendingImagePreview);
